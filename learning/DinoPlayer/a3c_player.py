...
path_work_dir = 'a3c_dino'
model_name = os.sep.join((path_work_dir, 'model'))
...
channels = 1
# 输入到神经网络的分辨率
resolution = (80, 80, channels)
# 一个动作包含的 帧
frames_per_action = 1  # 目前支持一帧以一个动作
# Chrome 驱动
...
def GrayToToHWD(img):
    return np.resize(img, resolution)
...
class ACNet(object):
    def __init__(self, num_actions, scope, trainer):
        with tf.variable_scope(scope):
            ...
            # 时间差分 RNN: Recurrent network for temporal dependencies
...
class Worker(object):
    def __init__(self, number, num_actions, trainer, model_name):
        """
        ctor
        :param number: 自 AC 网络的编号
        :param num_actions: 能够采取动作的数量
        :param trainer: 训练器对象
    ...
    def Process(self, sess, saver, coord):
        ...
        while (not coord.should_stop()):
            ...
            while (self.env.IsRunning()):
                ...
                image, r, finished = self.env.Act(a)
                image = GrayToToHWD(image)
                if (not finished):
                    s1 = image
                else:
                    s1 = backed
...
if __name__ == '__main__':
    ...
    env = EnvLab(chrome_driver=chrome_driver, show_image=True)
