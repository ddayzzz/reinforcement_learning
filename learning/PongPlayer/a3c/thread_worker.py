...
from a3c.utilities.trainer import AccumTrainer
from a3c.ac_network import ActorCriticLSTMNetwork
import gym
...
class A3CTrainingThread(object):
    ...
    def __init__(self,
        ...
        # 对 AC 网络的损失函数求导，同时初始化梯度累积变量
        self.trainer.prepare_minimize(loss=self.local_network.total_loss,
                                      var_list=self.local_network.get_vars())
        # 获取累积梯度的 op
        self.accum_gradients = self.trainer.accumulate_gradients()
        # 清空累积梯度的 op
        self.reset_gradients = self.trainer.reset_gradients()

        # 这个定义了所有在不同的网络中，变量名称相同的
            ...
            if terminal:
                ...
                sys.stdout.write("time %d | thread #%d | scene %s \n"
                                 ...
                                 "%s %s episode max Q  = %.3f\n" % (global_t, self.thread_index, self.scene_scope,
                                                                    self.scene_scope, self.task_scope,
                                                                    self.episode_reward,
                                                                    self.scene_scope, self.task_scope,
                                                                    self.episode_length,
                                                                    self.scene_scope, self.task_scope,
