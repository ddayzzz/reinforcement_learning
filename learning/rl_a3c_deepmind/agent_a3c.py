...
class ACNet(object):
    def __init__(self, num_actions, scope, trainer):
        with tf.variable_scope(scope):
            self.inputs = tf.placeholder(shape=[None] + list(resolution), dtype=tf.float32)  # resolution 估计是实际的 observation 的数量
            # CNN 的定义
            # 步长在两个维度上分别为2，2
            ...
            if (scope != global_scope_name):
                ...
                policy_loss = -tf.reduce_sum(tf.log(responsible_outputs) * self.advantages)  # log_pi(ai|si;theta_)。注意这里的 reduce_sum 是因为最终相乘的得到一个矩阵。
                ...
                self.apply_grads = trainer.apply_gradients(zip(grads, global_vars))  # zip 打包为列表，返回一个对 Zip 对象，可迭代，每个元素是一个 tuple，对应的位置的一个 tuple
