...
class AtariGameState(object):

    def __init__(self, env_script, display=False, crop_screen=True, frame_skip=4, no_op_max=30, output_video=False):
        """
        定义 agent
        :param display: 是否绘图
        :param env_script: GYM script
        ...
        self.env_id = env_script

        self.env = gym.make(env_script)
        # 引入监控器
